...
export function validateSQL(sql: string): ValidationResult {
  ...
  // Block multi-statements with semicolons (possible SQL injection). A single
  // semicolon at the very end of an otherwise single statement is allowed;
  // any other semicolon (whether mid-query or ending a statement that is
  // followed by more SQL) is not.
  const hasTrailingSemicolon = normalizedSql.endsWith(';');
  const sqlWithoutTrailingSemicolon = hasTrailingSemicolon
    ? normalizedSql.slice(0, -1).trimEnd()
    : normalizedSql;
  ...
  for (const keyword of dangerousKeywords) {
    ...
    // Keywords ending in "_" (xp_, sp_, pg_) are identifier prefixes rather
    // than standalone tokens, so only a leading word boundary is required;
    // every other keyword must match as its own token on both sides.
    const keywordPattern = keyword.endsWith('_')
      ? new RegExp(`\\b${escapedKeyword}`)
...
export function extractSQL(text: string): string | null {
  ...
  // Try to extract from a JSON structure first: a JSON payload's "sql" value
  // may itself contain a readable SELECT...FROM phrase, which the regex
  // below would otherwise match too greedily (including trailing JSON
  // syntax). Only fall back to the regex when this isn't parseable JSON
  // with a "sql" field.
  try {
