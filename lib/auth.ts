 ...
 */
export async function verifyAuth(req: NextApiRequest, res: NextApiResponse, next: () => void) {
  ...
  try {
    ...
    if (error.message?.includes('expired')) {
      return res
        .status(401)
        .json({ error: 'Token expired', message: 'Please obtain a new authentication token' });
    }
