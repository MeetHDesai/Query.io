...
export async function GET(req: Request) {
  // Confirmed correct export for Next.js API route
  try {
  ...
  try {
    ...
    if (!connectionString.startsWith('postgresql://')) {
      ...
      if (user && database) {
        ...
        return NextResponse.json(
          {
            error:
              'Missing required connection details. Either provide a full connection string or complete individual fields.',
          },
          { status: 400 }
        );
      }
    ...
    if (error instanceof Error) {
      ...
      if (error.message.includes('schema cache')) {
        errorMessage =
          'Database schema error. Please check that the connections table has the correct structure.';
