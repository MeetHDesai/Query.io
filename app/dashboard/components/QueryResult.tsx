...
export const QueryResultComponent = ({ result }: QueryResultProps) => {
  ...
  const isChartPossible =
    ...
    result.columns &&
    result.columns.length > 0; // Simplified check, specific checks in renderChart
    ...
    <Card className="bg-zinc-950 border border-zinc-800 rounded-xl p-3 sm:p-4 my-4 shadow-lg text-zinc-100 overflow-hidden">
      ...
      >
        ...
        <TabsContent value="table" className="mt-3 sm:mt-4">
                ...
                {!result.data || result.data.length === 0 ? (
                  <TableRow>
                    <TableCell
                      ...
                      className="text-center text-zinc-500 py-6 text-sm"
                    >
          ...
          {result.description && (
            <p className="mt-3 text-xs sm:text-sm text-zinc-400 italic">{result.description}</p>
          )}
        </TabsContent>
        ...
        <TabsContent value="chart" className="mt-3 sm:mt-4">
          {isChartPossible ? (
            <div className="h-[400px] sm:h-[450px] w-full flex flex-col bg-zinc-900 p-3 sm:p-4 rounded-lg border border-zinc-800">
              ...
              {result.title && (
                <h3 className="text-sm sm:text-base font-semibold mb-3 text-center text-zinc-100 truncate">
                  {result.title}
                </h3>
              )}
...
function renderChart(result: QueryResult) {
  ...
  if (!result.chartType || !result.data || !result.columns || result.data.length === 0) {
    return (
      <div className="flex items-center justify-center h-full">
        <p className="text-zinc-500">Insufficient data or configuration for chart.</p>
      </div>
    );
  }
  ...
  switch (result.chartType) {
    case 'bar':
      if (!xAxisKey || !primaryValueKey)
        return (
          <div className="flex items-center justify-center h-full">
            <p className="text-yellow-500">
              Bar chart requires at least 2 columns (category, value).
            </p>
          </div>
        );
      return (
        <RechartsBarChart data={result.data} margin={{ top: 5, right: 5, left: -10, bottom: 5 }}>
          {' '}
          {/* Adjusted margins */}
    ...
    case 'area': // Area chart uses similar setup to Line
      if (!xAxisKey || secondaryValueKeys.length === 0)
        return (
          <div className="flex items-center justify-center h-full">
            <p className="text-yellow-500">
              Line/Area chart requires at least 2 columns (X-axis, Y-axis).
            </p>
          </div>
        );
      ...
      return (
        <ChartComponent data={result.data} margin={{ top: 5, right: 15, left: -10, bottom: 5 }}>
          {' '}
          {/* Adjusted margins */}
    ...
    case 'pie':
      if (!xAxisKey || !primaryValueKey)
        return (
          <div className="flex items-center justify-center h-full">
            <p className="text-yellow-500">Pie chart requires at least 2 columns (name, value).</p>
          </div>
        );
      return (
    ...
    case 'radar':
      if (!xAxisKey || secondaryValueKeys.length < 2 || !primaryValueKey)
        return (
          <div className="flex items-center justify-center h-full">
            <p className="text-yellow-500">
              Radar chart requires at least 3 columns (subject, metric1, metric2...).
            </p>
          </div>
        );
      ...
      return (
        <RadarChart cx="50%" cy="50%" outerRadius="80%" data={result.data}>
          ...
          <PolarAngleAxis dataKey={xAxisKey} stroke="#a1a1aa" tick={tickStyle} />{' '}
          {/* This might need adjustment depending on data structure */}
          ...
          <Radar
            dataKey={
              primaryValueKey
            } /* Need to adapt dataKey based on how data is structured for Radar */
            stroke={COLORS[0]}
            fill={COLORS[0]}
            fillOpacity={0.6}
            name={formatColumnName(primaryValueKey)}
          />
    ...
    case 'radial':
      if (!xAxisKey || !primaryValueKey)
        return (
          <div className="flex items-center justify-center h-full">
            <p className="text-yellow-500">
              Radial chart requires at least 2 columns (category, value).
            </p>
          </div>
        );
      ...
      return (
        <div className="flex items-center justify-center h-full">
          <p className="text-zinc-500">
            Chart type &apos;{result.chartType}&apos; is not supported or not applicable.
          </p>
        </div>
      );
  }
