  ...
  const renderChart = () => {
    ...
    switch (chartMeta.type) {
      case 'line':
        return (
          <ResponsiveContainer width="100%" height="100%">
            <LineChart data={prepareChartData} margin={{ top: 10, right: 30, left: 0, bottom: 10 }}>
              <CartesianGrid strokeDasharray="3 3" stroke="#3f3f46" />
              <XAxis dataKey={xKey} tick={{ fill: '#a1a1aa', fontSize: 12 }} stroke="#52525b" />
      ...
      case 'bar':
        return (
          <ResponsiveContainer width="100%" height="100%">
            <BarChart data={prepareChartData} margin={{ top: 10, right: 30, left: 0, bottom: 10 }}>
              <CartesianGrid strokeDasharray="3 3" stroke="#3f3f46" />
              <XAxis dataKey={xKey} tick={{ fill: '#a1a1aa', fontSize: 12 }} stroke="#52525b" />
              <YAxis tick={{ fill: '#a1a1aa', fontSize: 12 }} stroke="#52525b" />
              <Tooltip
    ...
    <div className="space-y-4 w-full">
      ...
      {(title || summary) && (
        <div className="space-y-1">
          ...
          {activeView === 'table' && summary && <p className="text-sm text-gray-300">{summary}</p>}
        </div>
      )}
        ...
        <TabsContent value="chart" className="mt-3">
          <Card className="bg-zinc-900 border-zinc-800 p-4 rounded-md">
            <div className="h-[350px] w-full flex flex-col">
              ...
              {summary && <p className="text-sm text-zinc-400 mb-3 text-center">{summary}</p>}
